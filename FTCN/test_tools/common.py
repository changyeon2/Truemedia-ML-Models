...
def detect_all(file, sfd_only=False, return_frames=False, max_size=None, face_thres=0.5, bbox_thres=5000):
    frames = grab_all_frames(file, max_size=max_size, cvt=True)
    if not sfd_only:
        detect_res = flatten(
            [detector.detect(item) for item in partition(frames, 50)]
        )
        detect_res = get_valid_faces(detect_res, face_thres=face_thres, bbox_thres=bbox_thres)
    else:
