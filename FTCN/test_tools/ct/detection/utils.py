...
def get_valid_faces(detect_results, max_count=10, face_thres=0.9, bbox_thres=5000, at_least=False):
    new_results = []
    for i, faces in enumerate(detect_results):
        ...
        for j, face in enumerate(faces):
            bbox_area = np.abs(face[0][2] - face[0][0]) * np.abs(face[0][3] - face[0][1])
            if (face[-1] <= face_thres or bbox_area <= bbox_thres) and not (j == 0 and at_least):
                continue
            box, lm, score = face
